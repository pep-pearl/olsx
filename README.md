...
이 프로젝트는 OpenLayers를 완전히 숨기는 두꺼운 블랙박스 래퍼를 지향하지 않습니다. 자주 쓰는 지도 구성은 React스럽게 제공하고, 더 깊은 커스터마이징이 필요한 사용자는 OpenLayers 객체와 registry에 직접 접근할 수 있게 합니다.
...
- `mapRef`, `viewRef`, `layerRegistryRef`, `sourceRegistryRef`를 제공하는 refs context.
- `isMapReady`만 따로 구독할 수 있는 ready context.
- OpenLayers layer mount/cleanup/registry 등록을 공통화한 `useMountLayer`.
- 직접 tile source를 넘겨 사용할 수 있는 `OLSXTileLayer`.
- React children을 OpenLayers overlay로 띄우는 `OLSXOverlay`.
...
- 직접 OpenLayers를 제어할 수 있는 `useMapRefsContext`와 호환용 `useMapContext`.
...
import { OLSXMap, OLSXTileLayer } from "olx";
...
import { OLSXMap, OLSXOverlay, type OLSXOverlayRef } from "olx";
...
- `OLSXOverlay`: popup, label, callout처럼 지도 좌표에 붙는 React UI.

직접 OpenLayers를 다뤄야 한다면 낮은 수준의 확장 지점을 사용합니다.

- `useMapRefsContext()`로 OpenLayers map/view ref와 layer/source registry에 접근할 수 있습니다.
- `useMapReadyContext()`로 지도 준비 상태만 구독할 수 있습니다.
- `useMapContext()`는 refs와 ready 값을 함께 제공하는 호환용 hook입니다.
- `useMountLayer()`는 새 layer wrapper를 만들 때 registry 등록과 cleanup을 일관되게 처리하는 내부 기준 hook입니다.
- `OLSXTileLayer`는 source를 직접 넘기는 일반 tile layer wrapper입니다.
- `OLSXOverlay`는 `coordinate`, `visible`, `positioning`, `offset`, `autoPan`, `stopEvent`로 OpenLayers overlay 동작을 제어합니다.
- `createVectorLayer()`로 도메인 feature data 타입에 맞춘 typed compound component를 만들 수 있습니다.
- `BaseLayer`는 교체 가능한 preset입니다. 고급 사용자는 `useMapRefsContext`나 `useMountLayer`를 사용해 직접 layer 컴포넌트를 작성할 수 있습니다.
...
```tsx
import { BaseLayer, OLSXMap, OLSXOverlay, OLSXTileLayer, OLSXVectorLayer } from "olx";
```
...
- 고급 OpenLayers 동작은 현재 `useMapRefsContext`, `useMountLayer`, 또는 직접 OpenLayers 객체 접근 기반 custom component/hook으로 구현하는 것을 권장합니다.
