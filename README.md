...
- `mapRef`, `layerRegistryRef`, `sourceRegistryRef`를 제공하는 refs context.
...
- `useMapRefsContext()`로 OpenLayers map ref와 layer/source registry에 접근할 수 있습니다.
...
- `BaseLayer`는 교체 가능한 preset입니다. 고급 사용자는 `useMapRefsContext`를 사용해 직접 base layer 컴포넌트를 작성할 수 있습니다.
...
src/
  ...
  core/
    model/                         contexts and core model hooks
...
- 고급 OpenLayers 동작은 현재 `useMapRefsContext` 기반 custom component/hook으로 구현하는 것을 권장합니다.
